...
if(isset($_POST['search']))
{
   ...
   $title=$_POST['title'];
    ...
    $query = "SELECT p.id, p.title AS product_title,p.`description`,p.photo, p.`purchasing_price`, p.`salling_old_price`, p.`selling_new_price`, COALESCE(SUM(r.likes), 0) AS likes, COALESCE(SUM(r.stars), 0) AS stars, COALESCE(r.message,'') AS message FROM products p LEFT JOIN reviews r ON p.id = r.productid  WHERE p.`category` = '$category' OR p.`title` = '$category' GROUP BY p.id, p.title,p.`description`,p.photo, p.`purchasing_price`, p.`salling_old_price`, p.`selling_new_price`";
...
$query = "SELECT p.id, p.title AS product_title,p.`description`,p.photo, p.`purchasing_price`, p.`salling_old_price`, p.`selling_new_price`, COALESCE(SUM(r.likes), 0) AS likes, COALESCE(SUM(r.stars), 0) AS stars, COALESCE(r.message,'') AS message FROM products p LEFT JOIN reviews r ON p.id = r.productid   GROUP BY p.id, p.title,p.`description`,p.photo, p.`purchasing_price`, p.`salling_old_price`, p.`selling_new_price`";
 $result = $con->query($query);
...
<?php
  if ($result && $result->num_rows > 0) 
  {
    ...
    <div class="border">
        <h5 class="plant-text"><?php echo $row['product_title'] ?></h5>
        ...
        <p><i class="fa fa-thumbs-up" aria-hidden="true"></i> <?php echo $row['likes'] ?>  <i class='far fa-heart' style='font-size:24px;color:red'></i> <?php echo $row['likes'] ?></p>
